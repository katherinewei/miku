 ...
 */
define('h5/js/page/appCsadCommon', [
    'jquery',
    'h5/js/common/url',
    'h5/js/common/data',
...
], function($,URL, Data,CsadAddNotes,CsadUserMessage,csadQuestionnaireSurveyPage,CsadUserTrajectory,imageMagnification) {
    ...
    //删除数组指定的某个元素
    Array.prototype.indexOfC = function(val) {
    ...
    function bindEvents(){
        ...
        $('body')
            ...
            .on('contextmenu','.right.chatMess p3',function(e){
                var e = e || window.event;
                e.stopPropagation();
                var pointX = e.pageX;
                var pointY = e.pageY;
                var retreatsObj = $('.retreats');
                if(retreatsObj.length == 0){
                    $('body').append('<div class="retreats" style="position: absolute;left: '+pointX+'px;top: '+pointY+'px;">撤回消息</div>');
                }
                else{
                    retreatsObj.removeClass('hide').css({'left':e.pageX+'px','top':pointY+'px'});
                }
                var id = $(this).attr('data-id');
                retreatsObj.attr('data-id',id);
                return false;
            })
            .on('click','.retreats',function(){
                var $this = $(this);
                retreatsCmdMsg($this.attr('data-id'));
            })
    }
...
//处理不支持<audio>标签的浏览器，当前只支持MP3
    var playAudioShim = function ( dom, url, t ) {
        ...
        WebIM.utils.getIEVersion < 9 && audioDom.push(d);
        d.jPlayer({
            ready: function () {
            ...
            solution: (WebIM.utils.getIEVersion != 9 ? "html, flash" : "flash"),
            swfPath: "sdk/jplayer",
            supplied: "mp3",
            ended: function () {
                pause.hide();
    ...
    var uploadShim = function ( fileInputId, type ) {
        ...
        return new SWFUpload({
            ...
            file_dialog_start_handler: function () {
                if ( WebIM.utils.getIEVersion &&WebIM.utils.getIEVersion < 10 ) {
            ...
            upload_error_handler: function ( file, code, msg ) {
                if ( code != SWFUpload.UPLOAD_ERROR.FILE_CANCELLED
                    ...
                    this.uploadOptions.onFileUploadError && this.uploadOptions.onFileUploadError({ type: WEBIM_UPLOADFILE_ERROR, msg: msg });
                }
            },
...
//提供上传接口
    ...
    function handlePageLimit() {
        if ( WebIM.config.multiResources && window.localStorage ) {
...
//异常情况下的处理方法
    var handleError = function(e) {
        ...
        if (curUserId == null) {
            ...
            var msg = e.msg;
            if (e.type == WEBIM_CONNCTION_SERVER_CLOSE_ERROR) {
                if (msg == "" || msg == 'unknown' ) {
            ...
            } else if (e.type === WEBIM_CONNCTION_SERVER_ERROR) {
                            ...
                            $('.chat-content-p3 audio').each(function(){        //下载语音
                                ...
                                if(msgLocal.length && msgLocal.filename){
                                    ...
                                    options.onFileDownloadComplete = function(response, xhr) {
                                        ...
                                        if (WebIM.utils.getIEVersion != 9 && window.Audio) {
...
//显示当前选中联系人的聊天窗口div，并将该联系人在联系人列表中currentWin
    var showContactChatDiv = function(chatUserId,sid) {
        ...
        if(outer.find('#fillReportInchat'+numChatId).length == 0){
            outer.find('.rightNa').append('<iframe class="fillReportIframeChat" id="fillReportInchat'+numChatId+'" src="'+URL.questionnaireSurveyReportPage+'?id='+numChatId+'&name='+name+'&csadId='+curUserId+'&mobile='+mobile+'&isTemp='+isTemp+'&isNotReg=false&coopCode='+coopCode+'&forAPP=true"></iframe>');
        }
        else{
...
//表情选择div的关闭方法
    ...
    var sendText = function(msg,isQuickReply) {
        ...
        var msgB = new WebIM.message('txt', id);//创建文本消息

        msgB.set({
            msg: msg,
            to: to,
            success: function ( id,serverMsgId ) {console.log(id,serverMsgId)}//消息发送成功回调
        });

        if ( curChatUserId.indexOf(groupFlagMark) >= 0) {
            msgB.setGroup("groupchat");
            msgB.body.type = groupFlagMark;
            msgB.body.to = manageId(to);
            msgB.body.ext = {isExpert: 1};
        }
        ...
        var msgtext = WebIM.utils.parseEmoji(msg)
    ...
    var closeTalking = function() {
        ...
        var msgtext = WebIM.utils.parseEmoji(encode(msgB.body.msg.msg).replace(/\n/mg, ''))
        appendMsg(curUserId, to, msgtext,msgB.body.ext.MikuExpand);
        upDateStatus(to,2);
    ...
    function sendMessage(options,txt){
        ...
        conn.send(options);
        options.id = conn.getUniqueId();//生成本地消息id
        var msgtext = WebIM.utils.parseEmoji(encode(options.msg).replace(/\n/mg, ''))
        if(txt){
    ...
    //发送命令消息。显示是否正在输入中
    var sendActionMsg = function(msgStr,sendMan,curSessionId){
        ...
        if(msgStr == 'OnLine'){        //发送是否在线的消息
            msg.body.action = msgStr;
            msg.body.ext = '';

        }
        if (curChatUserId && curChatUserId.indexOf(groupFlagMark) >= 0) {
            msg.setGroup("groupchat");
            msg.body.to = manageId(to);
            msg.body.ext.isExpert = 1;
        }
        conn.send(msg.body);
...
//发送图片消息时调用方法
    var sendPic = function() {
        ...
        var input = document.getElementById('fileInput');//选择图片的input
            ...
            msg.set({
                ...
                onFileUploadComplete: function (data) {
                    var file = document.getElementById('fileInput'),
                        img;
                    if (WebIM.utils.isCanUploadFileAsync && file && file.files) {
        ...
        if (window.Notification) {
            ...
            var popNotice = function() {
                if (Notification.permission == "granted") {
                    var notification = new Notification("您有未读消息：", {
                        body: userNickName + '说: '+msg,
                        icon: imgPath+'common/images/desktoptip.gif',
                        renotify:true,
                        tag:'notification',
                        //silent:true,
...
//easemobwebim-sdk收到音频消息回调方法的实现
    var handleAudioMessage = function(message) {
        ...
        options.onFileDownloadComplete = function(response, xhr) {
            var len = response.size,
                objectURL = WebIM.utils.parseDownloadResponse.call(this, response);
            if (WebIM.utils.getIEVersion != 9 && window.Audio) {
                audio.onload = function() {
                    audio.onload = null;
                    window.URL && window.URL.revokeObjectURL && window.URL.revokeObjectURL(audio.src);
...
//显示聊天记录的统一处理方法
    var appendMsg = function(who, contact, message, onlyPrompt,messExcept,_timestamp) {
        ...
        if (typeof message == 'string') {
            localMsg = WebIM.utils.parseTextMessage(message);
        ...
        for (var i = 0; i < flg; i++) {
            ...
            if (type == "emotion") {
                // console.log(msg);
                var eletext = "<img src='" + data + "' class='emoji'/>";
                var ele = $(eletext);
                for (var j = 0; j < ele.length; j++) {
                ...
                //WebIM.utils.parseEmoji(this.encode(data).replace(/\n/mg, ''))
