...
define('h5/js/page/anonyCommon', [
    ...
    'h5/js/page/waittingH',
    'plugin/webIm/jplayer/jquery.jplayer.min',
    'h5/js/common/weixin',
], function($,URL, Data,HZRecorder,Strophe,EasemobShim,EasemobConfig,SWFUpload,imageMagnification,fastClick,getDetectRecord,waittingH,Weixin) {
    ...
    var csadRespone = true;//专家应答
    var needCallAgain = false;//需要再次呼叫
    //var isGetCsadStatus = true;//首次进入查看专家状态
    var csadStatus = 1;//默认专家在线
...
//easemobwebim-sdk注册回调函数列表
    ...
    function sendLeave(){
        if (conn) {
            ...
            conn.close();
            return navigator.userAgent.indexOf("Firefox") > 0 ? '确定是否退出 ?' : '';
        }
    }
...
//处理连接时函数,主要是登录成功后对页面元素做处理
    var handleOpen = function(conn) {
        ...
        if(!isRegist){
            // var chatName = groupSaveNH('chat','',true).csadName;
            $('.container').append('<section class="header"><div class="content navbar"><div class="btn-navbar navbar-left"><span href="javascript:history.go(-1);" class="icon icon-return"></span></div><div class="navbar-main">'+csadName+'</div><div class="btn-navbar navbar-right"><span class="register" href="'+URL.loginPage + '?sessionId='+sessionId+'&refurl=' + encodeURIComponent(location.href)+'">注册</span></div></div></section>');

            //处理高度
            $('.addBox').css('margin-top','45px');
            $('#chat01').height($('#chat01').height()-45);
            $('.chatExtend').height($('.chatExtend').height()-45);
        }

        //Common.headerHtml('免费咨询','<div class="btn-navbar navbar-right hide"><span  class="policy" href="'+URL.returnPolicy+'">注册</span></div>',false,$('.container'));
        ...
        //没登录给提示
        if(!isRegist){
            $('#chatWindow').append('<div class="waitExpert tc" href="'+URL.loginPage + '?sessionId='+sessionId+'&refurl=' + encodeURIComponent(location.href)+'">您当前尚未<span class="log">登录</span>，及时的登录有助于<p>专家更好的为您服务。</p></div>');
        }
    ...
    var getChatRecord = function(){
        ...
        if (chatRecord && chatRecord[0] == curUserId && chatRecord[2] == emExpertId) {
            ...
            $.each(chatRecord[1],function(i,msgLocal){
                ...
                if(from == curUserId){ //发送
                    ...
                    if(isRegist){
                        chatName=localStorage.getItem('csadName');
                        var currentUserIM=localStorage.getItem('currentUserIM');//获取缓存中注册用户信息
                        csadPicUrl=JSON.parse(currentUserIM)[4];//用户头像
                    }
                    appendMsg(from, msgtext,'',msgLocal,time,chatName,csadPicUrl);

                }
                else{  //接收

                    chatName = groupSaveNH(msgLocal.type,msgLocal).csadName;
                    csadPicUrl =  groupSaveNH(msgLocal.type,msgLocal).csadPicUrl;
                    if(msgLocal.ext && msgLocal.ext.MikuExpand){
                        msgtext = msgLocal.data;
                        appendMsg(from, msgtext,msgLocal.ext.MikuExpand,msgLocal,time,chatName,csadPicUrl);
                    }
                    else{
                        appendMsg(from, msgtext,'',msgLocal,time,chatName,csadPicUrl);
                    }
                ...
                $('.chat-content-p3 audio').each(function(){        //下载语音
                    ...
                    if(msgLocal.length && msgLocal.filename){
                        ...
                        options.onFileDownloadError = function(e) {
                            if(from == curUserId) { //发送
                                appendMsg(curUserId,  e.msg + ",下载音频" + filename + "失败",'','',time);
                            }else{
                                appendMsg(from,  e.msg + ",下载音频" + filename + "失败");
                            }
                        };
                        options.headers = {
                            "Accept" : "audio/mp3"
...
//登录系统时的操作方法
    ...
    //获取在线专家
    function getOnlineExpert(){
        var data = {
            // flag:2
        }
        Data.getRandomOneOnlineCsad(data).done(function(res){
            if(res){
                var flag = res.flag;
                $('.waittingHtml').remove();
                if(flag == 0){
                ...
                if(flag == 1){
                    bainx.broadcast('咨询人数已满，您前面还有'+res.size+'人在排队，请耐心等待');
                    setTimeout(function(){
                        getOnlineExpert();
                    },5000)
                    return false;
                ...
                $('.chatMain').hide();
                $('.chat02_bar').show();
                $('.chat02').css('height','auto');

                //$('.mTitle').css('background-color','#42454D');
                csadName=res.csadDO.csadName;//专家名
                if(!groupSaveNH('chat','',true).csadName){
                    $('.navbar-main').text(csadName);
                }
    ...
    function handleEstablishCasd(){
        ...
        $('.chatMain').hide();
        $('.chat02_bar').show();
        csadName=JSON.parse(currentCsadIM)[1];//专家名
        ...
        curChatUserId=userOneOnlineId;
        emExpertId=userOneOnlineId;
        // emExpertName = JSON.parse(currentCsadIM)[0]
    ...
    function groupLogin(){
        ...
        Data.getClientThreeTalkMikuEmGroup(data).done(function(res){
            ...
            loginImSystem(user,pass);
            $('.chat02_bar').show();
            emExpertId = vo.expertUserId;
            //csadName = vo.expertUserId;
        })
        //}
    }
    ...
    var showEmotionDialog = function() {
        ...
        $.each(data, function(k,v) {
            ...
            $('<li>').append(emotions).appendTo($('.swiper-slide1 ul'));
            //$('<li>').append(emotions).appendTo($('.wl_faces_main ul'));
        ...
        $('.wl_faces_main ul img').click(function(){
            selectEmotionImg(this);
        })
...
//表情选择div的关闭方法
    ...
    var sendText = function(msg) {
        ...
        if(needCallAgain){      //发送消息之前需要呼叫一次
            sendCallExpert();
            csadRespone=true;
            $('.serviceComment').parent().remove();
        }
        ...
        // 群组消息和个人消息的判断分支
            ...
            options = {
                to : curRoomId,
                msg : msg,
                ext :{groupName :groupName,coopCode : groupFlag,isExpert : 0},
                type : "groupchat"
            };
        ...
        options.success=function ( id,serverMsgId ) {
            console.log(id,serverMsgId)
        }//消息发送成功回调
        ...
        //getCsadStatus();
        appendMsg(curUserId,  msgtext,'','','',chatName,csadPicUrl);
        turnoffFaces_box();
        msgInput.value = "";
        setTimeout(function() {
            textSending = false;
    ...
    var sendCallExpert = function(msg) {
        if(!groupFlag){
            ...
            if(csadStatus == 2){        //专家离开状态
                options.ext.ExpertLeave = '1';
            }
    ...
    function sendCallExpertForChatEdForProject(){
        if(!groupFlag) {
            var currentUserIM=localStorage.getItem('currentUserIM'),//获取缓存中注册用户信息
                me=JSON.parse(currentUserIM[2]),
                options = {
                    to: curChatUserId,
                    msg: '描述：我是'+me+',我想解决的问题是'+ description + ',标题：' + title,
    ...
    function handleChatTimepPoor(){
        ...
        if($('.chatMess').last().length > 0){
            ...
            var timeStr =  $('.chatMess').last().find('.chatTime').data('timestape')==0 ? chatTimeQ : $('.chatMess').last().find('.chatTime').data('hisrecord') == 1 ? chatTimeQ : $('.chatMess').last().find('.chatTime').data('timestape'),
    ...
    function timer(){
        setInterval(function(){
            if(handleChatTimepPoor()>15 && timeFlag){
                ...
                $('#chatWindow').append('<div class="chatMess" style="text-align: center;"><p1 class="waitExpert tc">已经很久没收到您的消息了，系统将自动结束本次对话，如果您有其他问题，请<span class="reCallExp">重新接入</span>。</p1>'+commentTmp+'</div>');
                timeFlag=false;
            }
        },1000);
    }
...
//发送图片
        ...
        if (!Easemob.im.Helper.isCanUploadFileAsync || filetype in pictype) {
            var opt = {
                ...
                onFileUploadError : function(error) {
                    ...
                    //alert(opt.filename,opt.apiUrl,opt.to);
                    appendMsg(curUserId,  messageContent);
                },
                onFileUploadComplete : function(data) {
                    ...
                    if (curChatUserId.indexOf(groupFlagMark) >= 0) {
                        ...
                        saveData.to = curRoomId;
                        saveData.ext = {isExpert:0};
                        //to = manageId(to);
                    }
    ...
    //发送消息之前查看专家状态
    function getCsadStatus(){
        //查看专家在线状态
        //if(isGetCsadStatus){
        var emData = {
            emUserName:emExpertId
        }
        Data.isEmOnline(emData).done(function(res){
            //isGetCsadStatus = false;
            csadStatus = res.status;
            switch (res.status){
                case 0://离线
                    var currentCsadIM = localStorage.getItem('currentCsadIM'),
                        mobile = JSON.parse(currentCsadIM)[2];
                    $('#chatWindow').append('<div class="waitExpert tc" >hi~专家现在不在，如有急事请拨打<a href="tel:'+mobile+'" class="mobile">'+mobile+'</a>进行专家咨询。</div>');
                    break;
                case 2:
                    //var id = conn.getUniqueId();//生成本地消息id
                    //var msgB = new Easemob.im.EmMessage('txt', id);//创建文本消息
...
//easemobwebim-sdk收到文本消息的回调方法的实现
    var handleTextMessage = function(message) {
        ...
        if(!(message.ext && (message.ext.MikuExpand == 'ExpertRespond' || message.ext.MikuExpand == 'IsOver' || message.ext.MikuExpand == 'ExpertRespondLeave'))){
            //保存消息
            handleSaveData(message);
        }

        if(csadRespone && message.ext && message.ext.MikuExpand == 'ExpertRespond'){
            appendMsg(from,  messageContent,'','','',csadName,csadPicUrl);
            csadRespone = false;
        }
        if(!csadRespone && message.ext && message.ext.MikuExpand == 'ExpertRespond'){
            return false
        ...
        if(message.ext && message.ext.MikuExpand == 'UserReport'){
            ...
            appendMsg(from,  messageContent,message.ext.MikuExpand,message,'',csadName,csadPicUrl);
        }else if(message.ext && message.ext.MikuExpand == 'CheckBox'){
            appendMsg(from,  messageContent,message.ext.MikuExpand,message,'',csadName,csadPicUrl);
        }else if(message.ext && message.ext.MikuExpand == 'IsOver'){//专家主动结束聊天
            appendMsg(from,  messageContent,message.ext.MikuExpand,'','',csadName,csadPicUrl);
        }
    ...
    function handleSaveData(message){
        ...
        message.chat_timestamp = nowDate.getTime();
        ...
        if(!chatRecord){
            chatRecord = [message];
            chatRarr = [curUserId,chatRecord,emExpertId];
        }
        else{
            if(chatRecord[0] != curUserId || chatRecord[2] != emExpertId){
                chatRecord = [message];
                chatRarr = [curUserId,chatRecord,emExpertId];
            }
            else{
                ...
                chatRarr = [curUserId,chatRecord[1],emExpertId];
            }
    ...
    function deskupTips(msg){
        if (window.Notification) {
            ...
            console.log('浏览器不支持桌面提醒');
        }
    }


    //聊天的昵称头像存储
    function groupSaveNH(mestype,message,head){
        //接收处理是否注册用户还是匿名
        var csadNameQ,csadPicUrl;
        if(isRegist){
            var currentCsadIM=localStorage.getItem('currentCsadIM');//获取缓存中注册用户对应专家
            csadPicUrl=JSON.parse(currentCsadIM)[3];//专家头像
            csadNameQ=JSON.parse(currentCsadIM)[1];//专家名字
            emExpertId = JSON.parse(currentCsadIM)[0];
        }else {
            csadPicUrl=localStorage.getItem('csadPicUrl');
            csadNameQ=localStorage.getItem('csadName');
        }
        if(!head){
...
//统一的处理，不需要用户自己区别字符是文本还是表情符号。
    var handleEmotion = function(message) {
        ...
        //保存消息
        handleSaveData(message);
        var csadNameQ = groupSaveNH(mestype,message).csadName;
        var csadPicUrl =  groupSaveNH(mestype,message).csadPicUrl;

        appendMsg(from,  message,'','','',csadNameQ,csadPicUrl);
...
//easemobwebim-sdk收到音频消息回调方法的实现
    ...
    var handleCmdMessage = function(message){
        //console.log(message.ext.inputing);
...
//显示聊天记录的统一处理方法
    var appendMsg = function(who,  message, onlyPrompt,messExcept,_timestamp,chatName,csadPicUrl) {
        ...
        var hisRecord = _timestamp ? 1 : 0 ;
        var headstr;
        if(agentFlag=='true'){
            var img=csadPicUrl ? csadPicUrl : imgPath+'common/images/avatar9.png';
            headstr =(onlyPrompt && !messExcept) ? ['<p1 class="waitExpert tc">'+message+'</p1>'] : [ "<p2 class='chatTime "+showTime+"' data-hisrecord = '"+hisRecord+"' data-timestape='"+chatTimeQ+"'>" + chatTime + "<b></b><br/></p2>","<p1 class='userPhoto'><img src='"+img+"' /></p1>","<p class='chatName agent'>"+chatName+"</p>"];
        }else{
            headstr =(onlyPrompt  && !messExcept) ? ['<p1 class="waitExpert tc">'+message+'</p1>'] : [ "<p2 class='chatTime "+showTime+"' data-hisrecord = '"+hisRecord+"' data-timestape='"+chatTimeQ+"'>" + chatTime + "<b></b><br/></p2>","<p class='chatName'>"+chatName+"</p>"];
        }
    ...
    Easemob.im.EMOTIONS = {
        path: 'http://ninelab.b0.upaiyun.com/common/images/personalTailor/csad/faces/',
        map: {
            '[):]': 'ee_1.png',
            '[:D]': 'ee_2.png',
            '[;)]': 'ee_3.png',
