...
require([
    ...
    'jsPDF'
], function($,URL, Data,Common,ec,html2canvas,jsPDF) {
    var imgData,
        type = 'png',
...
// This can be changed to "pt" (points), "mm" (Default), "cm", "in"
    ...
    function render(){
        ...
        Data.getBoxRecordInfoByBoxId(data).done(function(res) {
            ...
            //基本信息
            $('#name').text(res.data.name ? res.data.name : '暂无');
            aname=res.data.name ? res.data.name : '';
            var sex='';
            if(res.data.sex){
                res.data.sex==1 ? sex='<span class="man"><img src="'+URL.site+'/image/icon_sure.png"></span>男<span class="woman"><img src="'+URL.site+'/image/pic_kuang.png"></span>女' : sex='<span class="man"><img src="'+URL.site+'/image/pic_kuang.png"></span>男<span class="woman"><img src="'+URL.site+'/image/icon_sure.png"></span>女';
            }else {
                sex='暂无';
            }

            $('#sex').html(sex);
            $('#weight').text(res.data.weight ? res.data.weight+'kg' : '暂无');
            $('#height').text(res.data.height ? res.data.height+'cm' : '暂无');
            if(res.data.birthday){
                $('#birthday').text(bainx.formatDate('Y-m-d', new Date(res.data.birthday)));
            }else {
                $('#birthday').text('暂无');
            }
            ...
            var qarr=[],h=[];
            if(res.data.expertCheckfaceQuestion){
                var expertCheckfaceQuestion=res.data.expertCheckfaceQuestion.split('\n');
                for(var p=0;p<expertCheckfaceQuestion.length;p++){
                    if(expertCheckfaceQuestion[p].length>42){
                        var reg=/.{1,42}/g,rs=expertCheckfaceQuestion[p].match(reg);
            ...
            var proImgsL=[];
            if(res.data.itemsName){
                //var proImgs=res.data.proImgs.split(';');
                var itemsName=res.data.itemsName.split(';');
                var imgU;
                for(var u=0;u<itemsName.length;u++){
                    if(itemsName[u]=='面部调理霜' || itemsName[u]=='修复面膜'){
                        ...
                        proImgsL.push('<li><div class="pic"><img src="'+imgU+'"></div><span>'+itemsName[u]+'</span></li>');
                    }
                }
            }
            ...
            var sarr=[],sh=[];
            if(res.data.suggest){
                var suggest=res.data.suggest.split('\n');
                for(var p=0;p<suggest.length;p++){
                    if(suggest[p].length>42){
                        var reg=/.{1,42}/g,rs=suggest[p].match(reg);
                        $.each(rs,function(i,v){
                            sh.push('<span>'+v+'</span>');
                        })
                    }else {
                        ...
                        data: [
                            {
                                value: skinCauseData,
                                name: '图一',
                                symbol: 'image://'+URL.site+'/image/pic_yuan.png',
                                symbolSize: 5,
                                lineStyle: {
                                    normal: {
                                        type: 'solid',
    ...
    function Events(){
        ...
        //下载
            .on('click','.download',function(){
                ...
                {
                    html2canvas(document.getElementById('skinReport'), {		//截屏
                        allowTaint: false,
                        taintTest: true,
