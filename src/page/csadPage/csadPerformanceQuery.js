...
define('h5/js/page/csadPerformanceQuery',[
    ...
    function echat(){
        ...
        $.each(navD,function(i,navli){
            navli.active = i == 0 ? 'active' : '';
            liHtml.push(bainx.tpl(tpl1,navli));
        })
            $('#performanceQuery_Wrap').html('<div class="searchU"><ul class="logNav">'+liHtml.join('')+'</ul></div><section><div class="searchGroup"><dl>'+html.join('')+'<dd>时间：<input type="text"  id="st" placeholder="起始时间" value="'+today+'" > 一 <input type="text"  id="et" placeholder="结束时间" value="'+today+'"  /><input type="button" value="查询" class="searchBtn" /> </dd></dl></div><div class="searchResult" ></div></section> ');
        //
    ...
    function getData(st,et,days){
        ...
        Data.sumTradeBoxPrice(data).done(function(res){
            ...
            if(legData.length > 0){
                ...
                $('#performanceQuery_Wrap').find('.boxList').scroll(function(){
                    var _this=$(this);
                ...
                $('#performanceQuery_Wrap').find('.boxList').html('<li class="noData">暂无</li>');
            }
    ...
    function getTableList(data,pg){
        ...
        Data.getBoxTradeVOList(data).done(function(res){
            ...
            var con  =$('#performanceQuery_Wrap').find('.boxList');
            if(pg == 0){
                con.html(html.join(''));
            }
            else{
