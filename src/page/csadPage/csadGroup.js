                        ...
                        time =  setTimeout(function(){
                           ...
                           //if($('#fillReport'+id).length == 0){
                            $('#fillReport'+id).remove();
                               CurrentPage.find('.rightCon').append('<iframe class="fillReportIframe" id="fillReport'+id+'" src="'+URL.questionnaireSurveyReportPage+'?id='+id+'&name='+name.text()+'&csadId='+csadId+'&isTemp=false&isNotReg=false&forAPP=false"></iframe>');
                           //}
                           //
                           //else{
                           //    $('#fillReport'+id).show();
    ...
    function getWechatUser(isWxBinding,pg,nicknameOrWxno,nickname,wxno){
        ...
        Data.getProfileWechatVOList(data).done(function(res){
                ...
                //点击下一页
                    ...
                    currentDiv.find('dl').append(dlHtml.join(''));
