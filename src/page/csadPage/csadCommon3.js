        ...
        $('body')
            ...
            .on('click','.deleteContactMan',function(){
                ...
                if($('#'+id).hasClass('currentWin')){
                    $('#chatMessage_Wrap').find('.mainContainer').hide();
                    $('#chatMessage_Wrap').find('#nullchater').show();
                }
                else{
                    $('#'+curUserId+'-'+id).remove();
                }
...
//登录系统时的操作方法
    ...
    function addQuickReplySentense(value,id){
        ...
        if($('#addReply').length == 0){
            $('.chat02').append('<div id="addReply" class=" grid addReplySentense" data-uid="'+manageId(curUserId)+'" '+idTmp+'><div class="pop_box"><div class="add_con"><input type="text" placeholder="输入标签" value="'+value+'"><p>请输入最多60个字</p></div><div class="pop_btn row fvc fac"><div class="col cancelBtn">取消</div><div class="confirmBtn  col">确定</div></div></div></div>');
        }
        else{
...
//构造陌生人列表
    var buildStrangerDiv = function(momogrouplistDivId,momogrouplistULId) {
        ...
        db.transaction(function (trans) {
            var sql = momogrouplistDivId == 'callCenter' ? 'select * from chatSession where  status = 0 and service_id = "'+user+'"  order by isTempEm desc, last_time desc' : 'select * from chatSession where  status > 0 and service_id = "'+user+'" and source = 1  order by read asc,isTempEm desc, last_time desc';
    ...
    function getItemUnreadNum(trans){
        trans.executeSql('select * from chatRecord where read = 0', [], function (ts2, data2)
        {
            if(data2){
                // var num = 0;
...
//显示当前选中联系人的聊天窗口div，并将该联系人在联系人列表中currentWin
    var showContactChatDiv = function(chatUserId,sid) {
        ...
        var outer = $('.leftNav li').eq(3).hasClass('active') ? $('#payVisit_Wrap') : $('#chatMessage_Wrap');
        ...
        if(outer.find('#csadUserMessageContainer_'+numChatId).find('.containerQuestion').children().length == 0){
            csadQuestionnaireSurveyPage(numChatId,'',$('.currentWin .message_main_item  span').text(),$('.currentWin .message_main_item  span').data('mobile'),curUserId,isTemp,$('.currentWin').attr('coopCode'));
        }
...
//easemobwebim-sdk收到文本消息的回调方法的实现
    ...
    function callToChat(who,isAnonymous,sid){
    ...
    function callTipHeadItem(isAnonymous,create,message,who,lielem,callTitle,temp_name){
        ...
        var chatWrap = $('#chatMessage_Wrap'),$whoc = chatWrap.find('#'+who);
        if(create){
...
//easemobwebim-sdk收到图片消息的回调方法的实现
    var handlePictureMessage = function(message) {
        ...
        var targetP =  $('#chatMessage_Wrap').find('#csadUserMessageContainer_'+_id).find('.questionAnalysis'),
            imgtpl = '<dd class="active fromIM"><img src="'+message.url+'"><i class="deleteImg"></i></dd>';
        ...
        $.each(membersList, function (i, item) {
            $.each(item.friendsGroupMapList, function (j, memberItem) {
                if (memberItem.emUserName == who) {
                    isNotcontains = 1;
                    spanelem.innerHTML = memberItem.userName ? memberItem.userName :  memberItem.profileName;
