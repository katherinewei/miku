...
define('h5/js/page/csadCreateMineBoxPage',[
   ...
   // 'plugin/addressData/1.0.0/addressData',
    ...
    var userBox = function(_uid,_rid,boxId,_csadName,_csadTel,canEdit,_userName,reportTime,notIframe,editBox,isReissue) {

        var currentExpert = sessionStorage.getItem('currentExpert');
        ...
        var Page,
            ...
            csadName = currentExpert ?currentExpert[0]:'',
            csadTel = currentExpert ?currentExpert[1]:'',
        ...
        function init() {
            ...
            if (_CanEdit) {
                if(editBox){
                    containerCreateBox.find('.'+containerBox).append('<div class="footer grid questionTool"><div class=""><span class="saveBoxBtn">修改</span></div></div>');
                }
                else{
        ...
        function getBoxDetailView(vo) {
            ...
            //  }
            var tpl = '<p>盒子生成时间：<span class="boxDateCreated">{{dateCreated}}</span></p><p>私人管家：' + csadName + '</p><p>联系方式：' + csadTel + '</p><p >盒子价格：￥<input  class="boxPrice price" value="{{price}}"/><span style="color: #ff4777;margin-left: 10px;font-size: 13px;">（建议零售价：<b class="suggestPrice price" >{{suggestP}}</b>）</span></p>',
                html = [];
            vo.dateCreated = bainx.formatDate('Y-m-d h:i', new Date(vo.dateCreated));
            vo.suggestP = !vo.price ? 0  : (vo.price / 100);
            vo.price = !vo.price ? '':(vo.price / 100);
        ...
        function createBoxProduct(target,lastCreateProduct) {
            ...
            Data.createOrUpdateMineBoxProduct(data).done(function (res) {
                ...
                containerCreateBox.find('.boxPrice').val('');
                containerCreateBox.find('.suggestPrice').text(sumPrice);
                //补发
                if(isReissue){
                    containerCreateBox.find('.boxPrice').val(0);
                }
        ...
        function deleteBoxProduct(target) {
            ...
            Data.deleteBoxProduct(data).done(function (res) {
                ...
                containerCreateBox.find('.chosedProductPrice').val((isNaN(res.boxPrice) ? 0 : (res.boxPrice / 100)));

                containerCreateBox.find('.boxPrice').val('');
                containerCreateBox.find('.suggestPrice').text((isNaN(res.boxPrice) ? 0 : (res.boxPrice / 100)));
                //补发
                if(isReissue){
                    containerCreateBox.find('.boxPrice').val(0);
                }
        ...
        function saveBox(target) {
            ...
            if(!$('.boxPrice').val() || (!isReissue && $('.boxPrice').val() == '0')){
                bainx.broadcast('请填写订单金额！');
                return false
            }
                ...
                Data.createOrUpdateBox(data).done(function (res) {
                    ...
                    $(window.parent.document).find('.sendBox').attr({'data-boxId':boxId,'data-boxName':res.vo.boxName,'data-boxImg':res.vo.picUrls})

                    $('.sendBox').removeClass('disabled');
                    isSave = true;
                    if(target.hasClass('sendBox')){
                        $('#coursePreview').remove();
                        $(window.parent.document).find('.sendBox').click();
                        return false;
                    }
                        ...
                        if(editBox){
                            ...
                            if(isReissue){
                                csadCreateOrder.prototype.init(_uid,res.vo.id,false,false,isReissue);
                            }else{
        ...
        function bindEvent() {
            ...
            $('body')
                ...
                //.off('click', '.sendBox').on('click', '.sendBox', function (event) {
                //    if($(this).hasClass('disabled')){
                //        bainx.broadcast('请先保存盒子');
                //        return
                //    }
                //    $(window.parent.document).find('.sendBox').click();
                //})
            ...
            containerCreateBox
                ...
                //保存盒子
                .off('click', '.saveBoxBtn,.sendBtnPreviewC,.sendBox').on('click', '.saveBoxBtn,.sendBtnPreviewC,.sendBox', function (event) {
                    var isSend = false;
                ...
                .off('click', '.numProdItem').on('blur', '.numProdItem', function (event) {
                    ...
                    if (!$(this).hasClass('disabled')) {
                        //库存
                        var quanti = parseInt(tarParent.find('.quantity span').text());
                        if(quanti <= parseInt($(this).val())){
