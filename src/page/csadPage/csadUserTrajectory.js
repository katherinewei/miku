...
define('h5/js/page/csadUserTrajectory',[
...
], function ($, Data, Common, URL,imageMagnification,getCsadPage,csadCreateMineBoxPage,csadCreateOrder,reissueOrder) {
    var body;
    var loginEnter = ['Android客户端','IOS客户端','官网','微信','微博'];//匿名用户登录方式
    var wraper;
    var statusP,sourceP;

    function initUserTrajectory(userId,outer,obj,isAppC){
        ...
        if(isTemp){
            ...
            getBoxList(0,10,userId,false,isAppC);//购买盒子记录
    ...
    //获取盒子订单
    function getBoxList(pg,sz,buyerid,showTr,isAppC){
        var data={
            type:1,
            allTrade:0,
            buyerId:buyerid,
        ...
        Data.getMineScBoxTradeList(data).done(function(res) {
            ...
            var tmp='<tr class=" ul_boxId{{id}}" data-id="{{id}}" data-tradeid="{{tradeId}}"><td width="5%" class="{{theFirst}}"><i>{{num}}</i></td><td width="10%" class="{{theFirst}}"><p  class="createTimeB">{{lastUpdated}}</p></td><td width="15%"><p>名称：{{boxName}}</p><p>{{time}}</p></td><td width="15%"><p>报告生成时间：</p><p>{{detectReportDate}}</p></td><td width="30%">{{product}}</td><td width="7%">￥{{price}}</td><td width="10%" class="payStatus"  data-tradeStatus="{{status}}"><span class="tradeStatus">{{tradeStatus}}</span>{{express}}</td><!--<td width="20%"><ul class="clearfix"><li class="fl">课程进度：</li><li class="fl"><p>共{{courseTimes}}分钟</p><p>第{{userStime}}/{{courseDuration}}天<p>已完成{{startedLessonTimes}}/{{totalLessonTimes}}次</p></li></ul></td>--><td width="7%">{{editBoxHtm}}<!--<a data-id="{{id}}" data-reportid="{{detectReportId}}"  class="viewDetail viewBox">查看产品>></a>{{placeOrderHtm}}<a  class="viewDetail viewReportU" data-id="{{detectReportId}}">查看报告>></a>--><a  class=" printReportBtn"  data-id="{{id}}">打印报告>></a> <a  class="{{hide}} reissueOrderBtn"  data-uid="{{userId}}" data-uname="{{userName}}" data-id="{{id}}" data-tradeid="{{tradeId}}">补发订单>></a></td></tr>';
            ...
            for(var j=0;j<arr.length;j++){
                    ...
                    $.each(arr[j],function(index,item){
                        res.theFirst = arr[j].length > 1 && index != 0  ? '' : 'theFirst';
                        ...
                        res.editBoxHtm = '';
                        res.placeOrderHtm = '';
                        res.tradeId = item.tradeId;
                        res.userId = item.userId;
                        res.userName = item.userName;

                        res.express = item.expressNO ? '<p class="express"><span>'+item.expressCompany+'</span>单号：'+item.expressNO+'</p>' : '';
                        var prodHtml=[],prodTpl='{{prodName}}×{{num}}';
                        $.each(item.productList,function(k,prodItem){
                            prodHtml.push(bainx.tpl(prodTpl,prodItem));
                        })
                        res.product = prodHtml.join(' ');
                        res.hide = '';
                        if(!(item.tradeStatus == 5 || item.tradeStatus == 7 || item.tradeStatus == 20)){
                            res.hide = 'hide';
                        }
                        ...
                        if(pg == 0 && !isAppC){
                            //var gData = {
                              //  userId:buyerid
                           // }
                            //Data.getNewestBox(gData).done(function(resN){
                               // var  boxId_n = resN.boxId
                                //if(resN.isOrder == 0 && boxId_n){
                                    //$('.buy_boxs_list tr').each(function(){
                                        //if(boxId_n == arr[j][0].id){

                                        //第一个订单
                                        var firstOrder = res.list[0];
                                        //未下单或已付款
                                         if(j == 0 &&  index == 0 && (firstOrder.tradeStatus == 4 || !firstOrder.tradeStatus)){
                                            var boxId_n = firstOrder.id,
                                                dpid_n = firstOrder.detectReportId,
                                                tradeid = firstOrder.tradeId ? firstOrder.tradeId : '',placeO='<a data-id="'+boxId_n+'"  class="placeOrderBtn">生成订单>></a>';
                                            if(tradeid){
                                                placeO='<a data-id="'+boxId_n+'" data-tradeid="'+tradeid+'"  class="placeOrderBtn">修改订单>></a>';
                                            }
                                             res.editBoxHtm='<a  class=" viewReportU" data-id="'+dpid_n+'">修改报告>></a><a data-id="'+boxId_n+'"   class="editBoxIt" data-reportid="'+dpid_n+'" data-tradeid="'+tradeid+'">修改产品>></a>'+placeO;
                                          //  return false;
                                        }
                                   // })
                                //}
    ...
    function Events(userId){
        wraper.find('#csadUserMessageContainer_'+userId)
            ...
            //修改报告
            .off('click','.viewReportU').on('click','.viewReportU',function(){
                $('.questionTab').text('用户诊断').addClass('activeTab').siblings().removeClass('activeTab');
                $('.presentList').addClass('hide');
                $('.presentList.containerQuestion').removeClass('hide');
            })
            // reportId = $(this).data('id');
            //    userName = wraper.find('#csadUserMessageContainer_'+userId).find('#userNameNote').text();
            //containerIframe.attr('src',URL.questionnaireSurveyPage+'?detectId='+reportId+'&userName='+userName+'&forAPP='+URL.param.forAPP);
        ...
        //生成订单
            ...
            csadCreateOrder.prototype.init(userId,_boxId,true);
            ...
            csadCreateMineBoxPage(userId, _rid, boxId, '', '', true, '', '', true,true);
