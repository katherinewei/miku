...
define('h5/js/page/csadQuestionnaireSurveyPage', [
    ...
    var questionSurvey = function(forApp,_uidO,rid,userName,userTel,serviceId,isAnonymous,coopCode,isNotReg)
    {
        ...
        function appendToMain(res,restart){
            ...
            if(res.tradeId && res.boxId){
                $('body').attr('data-tradeid',res.tradeId);
            }
        ...
        function submitQuestion(isdraft,target){
            ...
            var dataOrgin = {
               ...
               // expertCheckbodyQuestion:expertCheckbodyQuestion,
                remark:remark,
                serviceId:isNaN(serviceId) ? manageId(serviceId) : serviceId,
                cache:cache.join('%'),
           ...
           // console.log(cache.join('%'));
            ...
            if(isAnonymous){
                data.sessionId = _uid;
                Data.tempSaveOneTempRecord(data).done(function(res){
                    ...
                    //发送报告
                    if(target.hasClass('sendBtn')){
                        $(window.parent.document).find('.sendBtnActuely').attr({'data-rid':_rid,'data-name':userName,'data-rimg':container.find('.questionAnalysis dd.active').eq(0).find('img').attr('src'),'data-datecreated':_dateCreated}).click();
                        ...
                        if(!boxId){
                            ...
                            Data.createOrUpdateBox(boxData).done(function (resB) {
                                if(!target.hasClass('sendBtn')){
                                    ...
                                    $(window.parent.document).find('.sendBtnActuely').attr({'data-rid':_rid,'data-name':userName,'data-datecreated':_dateCreated}).click();
                                    $('.reportPreview').remove();
                                    return;
                                }
                                boxId = resB.vo.id;
        ...
        function bindEvents(){
            ...
            $(window.parent.document).on('click','.viewReportInChat',function(){
                var _ridChat = $(this).parent().data('id');
                viewReport(true,_ridChat);
            })
            ...
            $('body')
                ...
                .off('click','.submitBtnBox,.saveQuetionI,.sendBtn').on('click','.submitBtnBox,.saveQuetionI,.sendBtn',function(){
                if(canSubmit){
                    canSubmit = false;
                    var isdraf = true;
            ...
            container
                ...
                .off('click','.checkBox').on('click','.checkBox',function(){
                    ...
                    if((target.hasClass('selected')) && tarParent.hasClass('navLi')){
                        container.find('.addQue'+tarParent.index()).find('.checkBox').addClass('selected');
                    }
                    if(!tarParent.hasClass('navLi')){
                        ...
                        if(tarParent.hasClass('partition') || tarParent.hasClass('causedt')){
                            ...
                            if(target.hasClass('selected')){
                                ...
                                if(tarParent.next().next().next().length > 0){
                                    tarParent.next().next().next().find('.checkBox').addClass('selected');
                                }
                            }
                            else{
                                ...
                                if(tarParent.next().next().next().length > 0){
                                    tarParent.next().next().next().find('.checkBox').removeClass('selected');
                                }
                            }
