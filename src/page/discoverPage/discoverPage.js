...
require([
...
], function ($, Data, Common, URL,Slider,commentEntrance,shareCommon,Banner) {
    var userId = pageConfig.pid,ifclicks=false,iffollow=false,loadflag=false,loadmore=false,loadadd=false,isH5=URL.param.type,myfollow=URL.param.myfollow;
    ...
    function render(){
        ...
        //h5增加头尾
        if(isH5==1){
            var ht='<p class="choiceEntry"><span>精选</span><i></i></p>';
            Common.headerHtml('发现',ht);
            Common.renderAppBar();
        ...
        if(myfollow==1 && getSessionStrorage('typeflag')=='2'){
            setSessionStrorage('typeflag','1');//用来区分是精选文章还是关注文章，1是精选
            handleChoiceMyFollow();
        }else {
            if(res && flagState==1){//缓存
                bannerHtml(JSON.parse(getSessionStrorage('bannerlist')));
                //文章类型
                if(getSessionStrorage('typeflag')=='1'){
                    $('.choiceEntry span').text('精选');
                    $('.choice').addClass('active').siblings().removeClass('active');
                }else {
                    $('.choiceEntry span').text('我的关注');
                    $('.myConcern').addClass('active').siblings().removeClass('active');
                }
                articleList(res);
                $('.waitting').hide();
            }else{
        ...
        setSessionStrorage('flagState',1);
    ...
    function myHtmlItems(pg) {
        ...
        Data.personalContentPageNext(data).done(function(res) {

            articleList(res);

            var getPg=getSessionStrorage('pg'),
                getRes=getSessionStrorage('res');
            if(!getPg){
                setSessionStrorage('pg','0');
            }else{
                var pg=parseInt(getPg)+1;
            ...
            if(JSON.parse(getRes)){
                ...
                ////分开缓存toplist
                //setSessionStrorage('toplist',JSON.stringify(res.toplist));
            }
            setSessionStrorage('res',JSON.stringify(res));
            setSessionStrorage('typeflag','2');//用来区分是精选文章还是关注文章，1是精选
            loadadd && $('.add_more').eq(0).remove();
            loadadd=false;
        }).fail(function(){
            loadflag=false;
    ...
    function articleList(res){
        if ($.isArray(res.list) && res.list.length) {
            ...
            $.each(res.list, function (index, item) {
                ...
                var imgArr=mikuSnsContent.contentSurfacePicUrl.split(';'),
                    tp = [];
                if(imgArr.length==2){
                     //tp=[];
                    for(var f= 0;f<imgArr.length;f++){
                        tp.push('<div class="picBox w50"><span class="moreImg" style="background-image: url('+imgArr[f]+')"></span></div>');
                    }
                }else if(imgArr.length>2){
                    //var tp=[];
                    for(var f= 0;f<imgArr.length;f++){
                        tp.push('<div class="picBox"><span class="moreImg" style="background-image: url('+imgArr[f]+')"></span></div>');
                    }
                }else {
                    tp=['<div class="picBox w100"><span class="oneImg" style="background-image: url('+mikuSnsContent.contentSurfacePicUrl+')"></span></div>'];
                }
    ...
    //    //判断滑动方向
    //    if (y - startY>30) {//下滑
    //        var scrollTop = $('#indexPage').scrollTop();
    //        if(scrollTop<=20){
    //            handleDownRefresh(scrollTop);
    //        }
    //    }
    ...
    //
    ////处理下拉刷新
    //function handleDownRefresh(scrollTop){
    //    var sUserAgent=navigator.userAgent.toLowerCase();
    //    if(/android/.test(sUserAgent)){
    //        downLoadAnd(scrollTop);
    //    }
    //    if(sUserAgent.match(/iphone os/i) == "iphone os"){
    //        downLoadIos(scrollTop);
    //    }
    //}
    ...
    //处理我的关注切换操作
    function handleChoiceMyFollow(parameter){
        if(getSessionStrorage('typeflag')=='2'){
            return;
        }
        var res=JSON.parse(getSessionStrorage('res'));
        $('.article_list').empty();
        if(myfollow==1 && !parameter){//个人中心跳转并且不是点击我的关注
            $('.myConcern').addClass('active').siblings().removeClass('active');

            if(res && JSON.parse(sessionStorage.getItem('typeflag'))!=1){
                if($('#banner').children().length==0){
                    bannerHtml(JSON.parse(getSessionStrorage('bannerlist')));
                }
                articleList(res);
            ...
            parameter.addClass('active').siblings().removeClass('active');
            myHtmlItems(0);
        }

        if(pageConfig.pid==''){
            URL.assign('vLoginPage.htm?refurl='+URL.site+URL.discoverPage+'?type=1');
            return;
        }
    ...
    function bindEvent(){
        ...
        //精选切换
        $('.header')
            .on('tap','.choiceEntry',function(){
                if($('.popMask').css('display')=='none'){
                    $('.popMask').show();
        ...
        $('.popMask')
            .on('tap','.choice',function(){
                ...
                $('.choiceEntry').removeClass('active');
            ...
            .on('tap','.myConcern',function(){
                $('.choiceEntry').removeClass('active');
                handleChoiceMyFollow($(this));
        })
