...
require([
    ...
    function init(){
        $('body').append('<div id="csadUserMessageContainer_'+userId+'" class="createBoxContent"><div class="CreateBoxContainer"></div></div>');

        if(boxId){
            csadCreateMineBoxPage(userId,rid,boxId,csadName,csadTel,canEdit,userName,reportTime);
        }else{
            var data = {
                type:1,
                allTrade:0,
